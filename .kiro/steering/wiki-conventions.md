...
- Reports: `report-<slug>-YYYY-MM-DD.md` in `vault/outputs/reports/`
- Notes: `note-<slug>-YYYY-MM-DD.md` in `vault/outputs/notes/`
...
created: "YYYY-MM-DD"
updated: "YYYY-MM-DD"
source: "vault/staging/path/to/source.md"
confidence: high
---
```
...
- **high** — content derives directly from a source document in `vault/staging/`
...
- Every concept file MUST be reachable from `vault/wiki/index.md` in at most 2 hops

## Tag Registry (`vault/wiki/tags.yml`)

A canonical tag registry lives at `vault/wiki/tags.yml`. It is the single source of truth for all tags used in wiki frontmatter.

- Every tag in a `tags:` frontmatter field MUST exist as a canonical entry in `vault/wiki/tags.yml`.
...
Wiki pages may embed images from the `vault/asset/` folder when the source material contains meaningful visual content. Images are referenced using Obsidian-compatible wikilink syntax:

```
![[vault/asset/filename.png]]
```
...
```
![[vault/asset/02626f722fc3af1ea3d711589d9a0150_MD5.png]]
...
## Concept Files (`vault/wiki/concepts/`)
...
## Reference Files (`vault/wiki/reference/`)
...
- If the content explains an **idea, principle, or architecture** → concept file (`vault/wiki/concepts/`, 150 lines)
- If the content is a **syntax guide, keyword reference, or configuration cookbook** with substantial code examples → reference file (`vault/wiki/reference/`, 300 lines)
...
## Summary Files (`vault/wiki/summaries/`)
...
- If the content describes a **single atomic idea** → create a concept file in `vault/wiki/concepts/`
- If the content is a **syntax guide, keyword reference, or configuration cookbook** with substantial code examples → create a reference file in `vault/wiki/reference/`
- If the content **aggregates, compares, or connects multiple concepts** → create a topic file in `vault/wiki/topics/`
- **Never place wiki content files at the wiki root** — only `vault/wiki/index.md` lives there
...
Wiki pages may use Mermaid diagrams to illustrate processes, relationships, or structures when the content benefits from visual representation. Mermaid diagrams and static images (from `vault/asset/`) are equally valid ways to illustrate content — choose whichever fits the context best.
...
flowchart LR
    A[Staging Source] --> B[Summary]
```
*Figure: The ingest pipeline from staging source to wiki pages.*
````
...
- `vault/wiki/index.md` — catalog of all wiki pages, each with a `[[link]]` and a one-line summary, organized by category (Concepts, Summaries, Topics, Reference, Domains)
- `vault/wiki/index.md` MUST be updated after every ingest, file-back, or lint operation

## Domain Maps of Content (`vault/wiki/domains/`)
